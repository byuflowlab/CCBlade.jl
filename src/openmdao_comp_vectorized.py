...
class CCBladeWrapperComp(ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('v', shape=(num_nodes, 1), units='m/s')
        self.add_input('omega', shape=(num_nodes, 1), units='rad/s')
        self.add_input('hub_diameter', shape=(1, 1), units='m')
        self.add_input('prop_diameter', shape=(1, 1), units='m')
        ...
        self.add_output('normal_load_dist', shape=(num_nodes, num_radial), units='N')
        self.add_output('circum_load_dist', shape=(num_nodes, num_radial), units='N')
...
if __name__ == "__main__":
    ...
    comp.add_output('v', val=[10., 11.], shape=num_nodes, units='m/s')
    comp.add_output('omega', val=8000.*(2*np.pi/60), shape=num_nodes, units='rad/s')
    ...
    comp = CCBladeWrapperComp(num_nodes=num_nodes, num_radial=num_radial, B=B,
    ...
    for n in range(num_nodes):
        fig, ax = plt.subplots()
        ax.plot(radii, normal_load_dist[n, :], label='normal load')
        ax.plot(radii, circum_load_dist[n, :], label='circum load')
        ax.legend()
        fig.savefig(f'foo-{n}.png')
