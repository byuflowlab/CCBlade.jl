...
function CCBladeResidualComp(; num_nodes, num_radial, af, B, turbine, debug_print)
    ...
    for name in of_names
        push!(partials_data, PartialsData(name, "hub_radius", rows=rows, cols=cols))
        push!(partials_data, PartialsData(name, "prop_radius", rows=rows, cols=cols))
    ...
    for name in of_names
        push!(partials_data, PartialsData(name, "radii", rows=rows, cols=cols))
