...
class CCBladeJuliaTestCase(unittest.TestCase):
    ...
    def test_turbine_grant_ingram(self):
        ...
        comp = make_component(CCBladeResidualComp(num_nodes=num_nodes,
                                                  num_radial=num_radial, B=B,
                                                  af=affunc, turbine=turbine))
    ...
    def test_propeller_example(self):
        ...
        assert_allclose(prob.get_val('ccblade.Np', units='N/m')[0, :], Nptest, atol=1e-3, rtol=1)
        assert_allclose(prob.get_val('ccblade.Tp', units='N/m')[0, :], Tptest, atol=1e-3, rtol=1)
        assert_allclose((prob.get_val('ccblade.u', units='m/s')/Vinf)[0, :], unormtest, atol=1e-3, rtol=1)
        assert_allclose((prob.get_val('ccblade.v', units='m/s')/Vinf)[0, :], vnormtest, atol=1e-3, rtol=1)
    ...
    def test_propeller_aero4students(self):
        ...
        prob.model.add_subsystem(
            ...
            promotes_inputs=[('r', 'radii'), 'chord', 'theta', 'Vx', 'Vy',
                             ...
                             ('Rtip', 'prop_radius'), 'precone', 'pitch'],
    ...
    def test_camber(self):
        ...
        prob.model.add_subsystem(
            'ccblade', comp,
            promotes_inputs=[('r', 'radii'), 'chord', 'theta', 'Vx', 'Vy',
                             'rho', 'mu', 'asound', ('Rhub', 'hub_radius_eff'),
                             ('Rtip', 'prop_radius_eff'), 'precone', 'pitch'],
            promotes_outputs=['Np', 'Tp'])
        ...
        assert_allclose(prob.get_val('Np', units='N/m')[0, 3], 427.3902632382494, atol=1e-8, rtol=1)
        assert_allclose(prob.get_val('Tp', units='N/m')[0, 3], 122.38414345762305, atol=1e-8, rtol=1)
        assert_allclose(prob.get_val('ccblade.a')[0, 3], 2.2845512476210943, atol=1e-8, rtol=1)
        assert_allclose(prob.get_val('ccblade.ap')[0, 3], 0.05024950801920044, atol=1e-8, rtol=1)
        assert_allclose(prob.get_val('ccblade.u', units='m/s')[0, 3], 11.422756238105471, atol=1e-8, rtol=1)
        assert_allclose(prob.get_val('ccblade.v', units='m/s')[0, 3], 3.2709314141649575, atol=1e-8, rtol=1)
        assert_allclose(prob.get_val('ccblade.phi', units='rad')[0, 3], 0.2596455971546484, atol=1e-8, rtol=1)
        # assert_allclose(prob.get_val('ccblade.alpha', units='rad')[0, 3], 0.23369406105568025, atol=1e-8, rtol=1)
        assert_allclose(prob.get_val('ccblade.W', units='m/s')[0, 3], 63.96697566502531, atol=1e-8, rtol=1)
        assert_allclose(prob.get_val('ccblade.cl')[0, 3], 1.773534419416163, atol=1e-8, rtol=1)
        assert_allclose(prob.get_val('ccblade.cd')[0, 3], 0.03413364011028978, atol=1e-8, rtol=1)
        assert_allclose(prob.get_val('ccblade.cn')[0, 3], 1.7053239640124302, atol=1e-8, rtol=1)
        assert_allclose(prob.get_val('ccblade.ct')[0, 3], 0.48832327407767123, atol=1e-8, rtol=1)
        assert_allclose(prob.get_val('ccblade.F')[0, 3], 1.0, atol=1e-8, rtol=1)
        assert_allclose(prob.get_val('ccblade.G')[0, 3], 1.0, atol=1e-8, rtol=1)
    ...
    def test_multiple_adv_ratios(self):
        ...
        CTtest = np.array([
            ...
            -0.008450506440868786, -0.018336716057292365])[nstart:nend+1]
        CPtest = np.array([
            ...
            -0.004876593891333574, -0.013591625085158215])[nstart:nend+1]
        etatest = np.array([
