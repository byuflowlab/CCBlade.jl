...
Using this coordinate system (which is consistent with those used earlier in the paper), the normal force per unit length ($N^\prime$) is in the +x direction, and the tangential force per unit length ($T^\prime$) is in the -y direction.  If we define the coning angle along the blade as $\Phi(s)$ (add figure) then the thrust can be found through
integration as:
\begin{equation}
